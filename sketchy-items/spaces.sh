...
# Choose script based on FORCE_WM
if [ "$FORCE_WM" = "aerospace" ]; then
  ...
  SCRIPT_SPACES="export PATH=$PATH; $RELPATH/plugins/spaces/rift-script.sh"
else
...
if [ "$FORCE_WM" = "aerospace" ]; then
  ...
  # Get rift workspaces dynamically
  workspaces=$(rift-cli query workspaces 2>/dev/null | jq -r '.[] | .name')

  for workspace in $workspaces; do
    sketchybar --add item space.$workspace left \
      --subscribe space.$workspace rift_workspace_change \
      --set space.$workspace \
      icon="$workspace" \
      icon.padding_left=6 \
      icon.padding_right=7 \
      ...
      label.y_offset=-1 \
      label.drawing=off \
      click_script="$SCRIPT_SPACES $workspace" \
      script="$SCRIPT_SPACES $workspace"
  done
