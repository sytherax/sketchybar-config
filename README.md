...
{ config, inputs, pkgs, lib, ... }:
{
  ...
  config = lib.mkIf config.home-config.status-bar.enable {
    home.packages = with pkgs; [
      sketchybar-app-font
      # menubar-cli # (needs to come from an overlay, check [overlays/menubar-cli.nix](https://github.com/Kcraft059/Nix-Config/blob/master/overlays/menubar-cli.nix) for implementation - bin by @FelixKratz 
    ];
